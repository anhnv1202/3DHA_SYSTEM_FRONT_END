...
import { NotificationActionType, NotificationEpicType } from './notification.type';
import { storeNotifications } from './notification.action';
import NotificationService from '@app/services/http/notification.service';

export const fetchNotifications = (extras: { destroy$: Subject<void> }): AppAction => {
  return {
...
const fetchAndStoreNotificationsEpic = (action$: Observable<AppAction>): Observable<AppAction> => {
  return action$.pipe(
